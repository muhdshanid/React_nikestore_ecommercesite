...
const Cart = () => {
  ...
  return (
    <div>
      ...
      w-full h-screen opacity-100 z-[250] ${ifCartState ? 'opacity-100 visible translate-x-0' : 'opacity-0 invisible translate-x-8'}`}>
        <div className={`blur-effect-theme h-screen max-w-xl w-full absolute right-0`}>
        <CartCount onCartToggle={onCartToggle} onClearCartItems ={onClearCartItems} totalQTY={totalQTY}/>
          {cartItems?.length === 0 ? <CartEmpty onCartToggle={onCartToggle}/> : 
          <div>
            <div className='flex  flex-col gap-y-7 lg:gap-y-5 overflow-y-scroll h-[81vh] scroll-smooth scroll-hidden py-3'>
            ...
            <div className='fixed bottom-0  bg-white w-full px-5 py-2 grid items-center '>
              ...
              <div className='grid items-center gap-2'>
                ...
                <button type='button' className='button-theme bg-theme-cart text-white'>Checkout</button>
              </div>
            </div>
          </div>
          }
