...
const Item = ({ifExists, id,color,shadow,title,text,img,btn,rating,price }) => {
  ...
  //console.log(id);
    ...
    <div >
        ...
        hover:scale-105`}>
            <div className={`grid items-center ${ifExists ? 'justify-start' : 'justify-center'}  `}>
                ...
                <div className='flex items-center justify-between w-28 my-2'>
                    ...
                    px-1 rounded blur-effect-theme'><h1 className='text-black text-sm font-medium '>${price}</h1></div>
                    <div className='flex items-center gap-1'><StarIcon className='icon-style w-5 h-5 md:w-4 md:h-4 '/><h1 className='
                    font-normal text-slate-100 md:text-sm '>{rating}</h1></div>
                </div>
                <div className='flex items-center gap-3'>
                    ...
                    <button onClick={()=>{onAddToCart();onCartToggle();}} type='button' className='bg-white opacity-90 blur-effect-theme button-theme p-0.5 shadow shadow-sky-200 text-sm text-black
                    px-2 py-1'>{btn} </button>
                </div>
            </div>
            <div className={`flex items-center ${ifExists ? 'absolute top-5 right-1' : 'justify-center'}`}>
                <img className={` transitions-theme hover:-rotate-12
                 ${ifExists ? 'h-auto w-64 lg:w-56 md:w-48 -rotate-[35deg]' : 'h-36 w-64'}`} src={img} alt={`img/item-img/${id}`} />
            </div>
        </div>
    </div>
  )
